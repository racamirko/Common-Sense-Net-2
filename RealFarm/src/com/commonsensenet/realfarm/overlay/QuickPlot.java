 ...
 */
public class QuickPlot extends CustomPopupWindow {
	...
	private void createDiaryList() {
		...
		for (int i = 0; i < diaryList.size(); i++) {
			title = diaryList.get(i).getTitle();
			icon = diaryList.get(i).getIcon();
			listener = diaryList.get(i).getListener();
			id = diaryList.get(i).getId();
	...
	private View getActionItem(String title, Drawable icon,
		...
		LinearLayout container = (LinearLayout) inflater.inflate(
				R.layout.plot_item, null);

		ImageView img = (ImageView) container.findViewById(R.id.iconplot);
		TextView text = (TextView) container.findViewById(R.id.titleplot);
