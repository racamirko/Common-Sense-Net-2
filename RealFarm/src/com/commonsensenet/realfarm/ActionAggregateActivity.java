...
public class ActionAggregateActivity extends AggregateMarketActivity implements
	...
	protected void makeAudioAggregateMarketItem(AggregateItem item,
		...
		case RealFarmDatabase.ACTION_TYPE_FERTILIZE_ID:
			...
			// TODO AUDIO: Say something here: say(total) + " farmers growing "
			// + variety + " have applied " + fertilizer +
			// " their parcels this season." + say(number) +
			// "have done it in the last" +
			// say(RealFarmDatabase.NUMBER_DAYS_NEWS) +
		...
		case RealFarmDatabase.ACTION_TYPE_HARVEST_ID:
			...
			// TODO AUDIO: Say something here: say(total) + " have harvested " +
			// variety + " at an average yield of " + amount +
			// " quintal per acre this season." + say(number) +
		...
		case RealFarmDatabase.ACTION_TYPE_IRRIGATE_ID:
			int irrigation = mDataProvider.getResourceImageById(
					...
					RealFarmDatabase.COLUMN_NAME_RESOURCE_AUDIO);
			// TODO AUDIO: Say something here: say(total) + " farmers growing "
			// + variety + " have " + irrigation + " their parcels this season."
			// + say(number) + "have done it in the last" +
			// say(RealFarmDatabase.NUMBER_DAYS_NEWS) +
		...
		case RealFarmDatabase.ACTION_TYPE_REPORT_ID:
			int problem = mDataProvider.getResourceImageById(
					item.getSelector2(), RealFarmDatabase.TABLE_NAME_RESOURCE,
					RealFarmDatabase.COLUMN_NAME_RESOURCE_AUDIO);
			// TODO AUDIO: Say something here: say(total) + " have reported " +
			// problem + " for " + variety + " this season." + say(number) +
			// "have reported it in the last" +
			// say(RealFarmDatabase.NUMBER_DAYS_NEWS) +
		...
		case RealFarmDatabase.ACTION_TYPE_SELL_ID:
			...
			// TODO AUDIO: Say something here: say(total) + " people have sold "
			// + variety + " at prices between " + say(min) + " and " +
			// say(min+RealFarmDatabase.SELLING_AGGREGATE_INCREMENT) +
			// " rupees per quintal " + " this season." + say(number) +
		...
		case RealFarmDatabase.ACTION_TYPE_SOW_ID:
			int treatment = mDataProvider.getResourceImageById(
					item.getSelector2(), RealFarmDatabase.TABLE_NAME_RESOURCE,
					RealFarmDatabase.COLUMN_NAME_RESOURCE_AUDIO);
			// TODO AUDIO: Say something here: say(total) +
			// " farmers have sown " + variety + "and" + treatment +
			// " the seeds this season." + say(number) +
			// "have done it in the last" +
			// say(RealFarmDatabase.NUMBER_DAYS_NEWS) +
		...
		case RealFarmDatabase.ACTION_TYPE_SPRAY_ID:
			...
			int pesticide = mDataProvider.getResourceImageById(
					...
					RealFarmDatabase.COLUMN_NAME_RESOURCE_AUDIO);
			// TODO AUDIO: Say something here: say(total) + " have reported " +
			// prob + " for " + variety + " and used " + pesticide +
			// " as medicine this season." + say(number) +
			// "have done it in the last" +
			// say(RealFarmDatabase.NUMBER_DAYS_NEWS) +
