...
public class DummyHomescreenData extends BaseAdapter {
	...
	public void generateDummyItems(int numOfItems,
			Vector<Recommendation> infoCont) {
		Random rn = new Random();
		// get maximum numbers for each type
		int maxAct = mDataProvider.getActionNames().size() - 3;
		int maxSeed = mDataProvider.getSeeds().size() - 3;
		// generation
		infoCont.clear();
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) { // if it's not recycled, initialize some
									// attributes
			element = mInflater.inflate(R.layout.vi_recommendation, parent,
					false);
		} else {
			element = convertView;
		}
		...
		lblDesc.setText(mDataProvider.getActionNameById(tmpRec.getAction())
				.getName());
		lblDetail
				.setText(mDataProvider.getSeedById(tmpRec.getSeed()).getName());
		imgDesc.setImageResource(mDataProvider.getActionNameById(
