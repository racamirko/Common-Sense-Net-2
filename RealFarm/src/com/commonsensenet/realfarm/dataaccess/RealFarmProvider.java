...
public class RealFarmProvider {
	...
	// "/Android/data/com.commonsensenet.realfarm/files";
	File file, file1;
	 ...
	 * 
	 * List<Plot> plots = getPlotsByUserId(userId); for(int x = 0; x <
	 * plots.size(); x++) {
	 ...
	 * 
	 * final String SQL =
	 * "SELECT a.%s, a.%s, a.%s, a.%s, a.%s, a.%s, a.%s FROM " +
	 * RealFarmDatabase.TABLE_NAME_ACTION + " a INNER JOIN " +
	 * RealFarmDatabase.TABLE_NAME_GROWING + " b ON a.%s = b.%s WHERE b.%s = " +
	 * plotId;
	 * 
	 ...
	 * if (c.getCount() > 0) {
	 * 
	 * c.moveToFirst(); do { tmpList.add(new Action(c.getInt(0), c.getInt(1),
	 * c.getInt(2), c .getInt(3), c.getInt(4), c.getInt(5), c.getString(6)));
	 * 
	 * } while (c.moveToNext()); }
	 * 
	 * c.close(); mDb.close(); return tmpList;
	 * 
	 * }
	 */
	public List<Growing> getGrowingsByUserId(int userId) {
		...
		List<Plot> tmpPlots = getAllPlotListByUserId(userId);
	...
	public List<Plot> getAllPlotList() { // modified
		...
		if (c.getCount() > 0) {
			do {
				plotList.add(new Plot(c.getInt(0), c.getInt(1), c.getInt(2),
						c.getInt(3), c.getInt(4), c.getString(5), c
								.getString(6), c.getInt(7), c.getInt(8)));
	...
	public long setActionNew(int actionid, int actionnameid, int growingid,
	...
	public Cursor getActionTranslation() {
		...
		Cursor c = mDb
				.getAllEntries(
						RealFarmDatabase.TABLE_NAME_ACTIONTRANSLATION,
						new String[] {
								RealFarmDatabase.COLUMN_NAME_ACTIONTRANSLATION_ACTIONNAMEID,
		...
		if (c.getCount() > 0) {
			...
			do {
				...
				String log = "ACTIONNAMEID: " + c.getInt(0)
						+ " ,TARGETIDFIELD: " + c.getString(1)
						+ " ,TARGETTABLE: " + c.getString(2)
						+ ",INDOBJECTIDFIELD: " + c.getString(3)
						+ " ,INDOBJECTTABLE: " + c.getString(4)
	...
	public List<Plot> getAllPlotListByUserId(int userId) { // modified
	...
	public List<Plot> getAllPlotListByUserIdPlotId(int userId, int plotId) { // modified(You
																				// can
																				// take
																				// seedtypyId
																				// corresponding
		...
		if (c.getCount() > 0) {
			do {
				plotList.add(new Plot(plotId, userId, c.getInt(0), c
	...
	public List<Plot> getPlotDelete(int delete) {
		...
		if (c.getCount() > 0) {
			do {
				tmpList.add(new Plot(c.getInt(0), c.getInt(1), c.getInt(2),
						c.getInt(3), c.getInt(4), c.getString(5), c
								.getString(6), delete, c.getInt(7)));
	...
	public List<Plot> getAllPlotListByUserDeleteFlag(int userId, int delete) { // modified(You
																					// can
																					// take
																					// seedtypyId
																					// corresponding
