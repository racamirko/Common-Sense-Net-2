...
public class RealFarmDataSynchronizationService extends BroadcastReceiver {
    ...
    //        Log.d("8 sep: ", separated[7]); 
            ...
            // Sensed Data format - Type, 1, 25,0-10, 2, 25, 0-10, 1, rain, val, MP, GN, 3400, SM, SM1, SM2, Cluster 
            // 1- today, 2-tomorrow
           // 1 1 25 1 2 25 2 1 rain 2 MP GN 3400 SM 56 23 1
            ...
            // 5 = Updates to users, action types, units, plots, pesticides, fertilizer, seeds, stages etc..
            ...
            //  Its a WF, MP, SM message
            	...
            	Log.d("after int conversion ", "after"); 
            	 String type1 = null;
            	switch(condval){
            	case 0:  type1="Chance of rain"; break;
            	case 1:  type1="Cloudy "; break;
            	case 2:  type1="Fog"; break;
            	case 3:  type1="Sunny"; break;
            	case 4:  type1="Rain"; break;
            	case 5:  type1="Partly Cloudy"; break;
            	}
            	...
            	int condval1= new Integer(cond1);
            	 String type2 = null;
            	switch(condval1){
            	case 0:  type2="Chance of rain"; break;
            	case 1:  type2="Cloudy "; break;
            	case 2:  type2="Fog"; break;
            	case 3:  type2="Sunny"; break;
            	case 4:  type2="Rain"; break;
            	case 5:  type2="Partly Cloudy"; break;
            	}
            	Log.d("cond1 val: ", type2); 
        /*    	String cond2=separated[11];
            	int condval2= new Integer(cond2);
            	String type3 = null;
            	switch(condval2){
            	case 0:  type3="Sunny"; break;
            	case 1:  type3="Clear"; break;
            	case 2:  type3="Overcast"; break;
            	case 3:  type3="Mostly Cloudy"; break;
            	case 4:  type3="Partly Cloudy"; break;
            	}*/
            	
            // Msg to display from www.tititudorancea.com
            	String wfmsg= "Today's forecast in CKP is " + type1 + " with Max Temperature around " + separated[3] + "\u00b0 C and Tomorrow's forecast is " + type2 + " with temperature around" 
            			+ separated[6] + "\u00b0 C - tititodurecea.com";
            	 Toast.makeText(context, wfmsg, Toast.LENGTH_SHORT).show();
           // Msg to display from Rain gauge in CKP
            	String wfmsg2= " In CKP " +separated[10]+ " mm of rain was noticed yesterday.";
            	Toast.makeText(context, wfmsg2, Toast.LENGTH_SHORT).show();
           // Msg for Market Price 	""
            	if(separated[11].toString().equalsIgnoreCase("MP"))
                 {
                   	 String mpmsg= "Today's Groundnut price in Pavagada is " + separated[13] + " Rs";
                 	 Toast.makeText(context, mpmsg, Toast.LENGTH_SHORT).show();
                 }
            	if(separated[14].toString().equalsIgnoreCase("SM"))
            	{
            		String smmsg= "Today's Soil Mositure values are " + separated[15] + " and " + separated[16] + "% from cluster" + separated[17];
            		Toast.makeText(context, smmsg, Toast.LENGTH_SHORT).show();
            	}
 	          ...
 	          int wf_size = mDataProvider.getWFData().size() +1;
 	           mDataProvider.setWFData(wf_size,separated[2], wf_val, type1, separated[5], wf_val1, type2, admin_flag); //int WF_ID, String WF_Date, int WF_Value, int WF_Type, int WF_Date1, int WF_Value1, int WF_Type1, int WF_adminflag)
            ...
            if(typeval == 2)
            {
            // For Actions taken a message has to be sent from the farmers phone to Server
          //  	ActiontypeId, User Id, localId, Quantity, units, Date, Crop or plot. 
            	// To be on the server
            	 if(separated[2].toString().equalsIgnoreCase("AT"))
              	{
            		  String[] actionsdone = str.split("/");
            		  String atmsg= "User has performed some actions: localATID " + actionsdone[1] + "second " + actionsdone[2] + "third " +actionsdone[3];
            		 
              	//	String atmsg= "User has performed some actions: localATID " + separated[3] + " Gowing ID:  " + separated[4] + " Action name id: " 
              	//+ separated[5] + " quantity " + separated[6] + " unit id: " + separated[7] + " quantity 2 " + separated[8] + " Date " + separated[9];
            ...
            if(typeval == 5)
            {
            // For updating users, action types, units, plots, pesticides, fertilizer, seeds, stages...
            	// users= subtype, first name, last name, mobileno, plotid, long, lat ( users, plots)
          	 ...
          	 if(separated[2].toString().equalsIgnoreCase("US"))
             	{
          		Log.d("subtype: ", "in US"); 
             		String usmsg= "Updation to user table Firstname: " + separated[3] + " Last Name: " + separated[4] + " Mobile No: " 
             	+ separated[5] ;//+ " and his plot longitude and latitude values are " + separated[6] + ", " + separated[7];
             		Toast.makeText(context, usmsg, Toast.LENGTH_SHORT).show();
       ...
       //      		mDataProvider.getUser();
             	...
             	Toast.makeText(context, insertedtodb, Toast.LENGTH_SHORT).show();
             	} 
            	// For Action type updates
             	...
             	{
             		String acmsg= "Updation to Actions Taken table-  Action name to be added is : " + separated[3];
             		Toast.makeText(context, acmsg, Toast.LENGTH_SHORT).show();
             	} 
            	 
            	 if(separated[2].toString().equalsIgnoreCase("UT"))
              	{
              		String utmsg= "Updation to Units table-  unit name to be added is : " + separated[3] + " and value is " + separated[4];
         ...
         //    		mDataProvider.getUser();
            // 		String insertedtodb="inseted to DB";
            //   		Toast.makeText(context, semsg, Toast.LENGTH_SHORT).show();
               	} 
            	
            	 if(separated[2].toString().equalsIgnoreCase("PE"))
                	{
                		String pemsg= "Updation to pesticides table-  pesticide name to be added is : " + separated[3];
                		Toast.makeText(context, pemsg, Toast.LENGTH_SHORT).show();
                	} 
            	 
            	 if(separated[2].toString().equalsIgnoreCase("FE"))
                	{
                		String femsg= "Updation to fertilizer table-  fertilizer name to be added is : " + separated[3] + "  , stage_id " + separated[4] +  " and units is  " + separated[5];
                		Toast.makeText(context, femsg, Toast.LENGTH_SHORT).show();
                	} 
            	 if(separated[2].toString().equalsIgnoreCase("ST"))
             	{
             		String stmsg= "Updation to stages table-  stage name to be added is : " + separated[3];
             		Toast.makeText(context, stmsg, Toast.LENGTH_SHORT).show();
             	} 
            }
        /*    
